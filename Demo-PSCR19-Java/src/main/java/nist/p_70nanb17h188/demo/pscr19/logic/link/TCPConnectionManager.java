...
//        void onServerSocketChannelCloseFailed(@NonNull ServerSocketChannel serverSocketChannel);
    ...
    TCPConnectionManager() throws IOException {
...
//    }
    ...
    SocketChannel addSocketChannel(@NonNull InetSocketAddress remoteAddress, @Nullable SocketChannelEventHandler socketChannelEventHandler) {
        ...
        try {
            ...
            if (key != null) synchronized (selector) {
                key.cancel();
            }
...
//            Log.d(TAG, "key size: %d", selector.keys().size());
    ...
    private void read(@NonNull SelectionKey key) {
        ...
        try {
            ...
            } // else the other side closed the socket (numRead < 0)
        } catch (IOException e) {
...
class SocketChannelBufferHandler {
    private static final String TAG = "SocketChannelBufferHandler";
    ...
    @Nullable
    final TCPConnectionManager.SocketChannelEventHandler socketChannelEventHandler;
    @NonNull
    ...
    SocketChannelBufferHandler(@NonNull SocketChannel socketChannel, @Nullable TCPConnectionManager.SocketChannelEventHandler socketChannelEventHandler) {
